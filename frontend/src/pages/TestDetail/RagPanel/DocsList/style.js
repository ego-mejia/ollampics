import tw from "tailwind-styled-components";

export const Wrap = tw.div``;

export const DocRow = tw.div`
  border-t border-border-soft py-3 px-4
`;

export const DocHeader = tw.div`flex items-baseline justify-between gap-3`;

export const DocName = tw.code`
  font-mono text-sm text-fg
`;

export const DocMeta = tw.span`
  text-xs text-fg-dim tabular-nums whitespace-nowrap
`;

export const Preview = tw.p`
  text-sm text-fg-muted mt-1 leading-relaxed
`;

export const ToggleBtn = tw.button`
  text-xs text-brand-blue mt-2 hover:underline
`;

export const FullContent = tw.pre`
  bg-elev/40 border border-border-soft rounded p-3 mt-2
  text-xs text-fg whitespace-pre-wrap font-mono leading-relaxed
  max-h-96 overflow-y-auto
`;

export const Empty = tw.p`
  text-sm text-fg-dim italic p-4 text-center
`;
